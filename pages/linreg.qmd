...
head(df_te)
```
Each row is a data point, and the columns represent the x and y coordinates. In case you haven't picked up on it yet, `tr` will be a suffix referring to the **tr**aining data, and `te` will be a suffix referring to the **te**sting data.
...
plt_tr + plt_te                         # <10>
```
1. Load the `ggplot2` and `patchwork` libraries. We'll be using these to produce plots and display them.
